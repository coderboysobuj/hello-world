...
namespace mmo::render {
    ...
    void VulkanBackend::RenderEntities(mmo::ecs::World& ecsWorld) {
        ...
        for (auto entity : ecsView) {
            ...
            if (meshComp.texture && meshComp.texture->descriptorSet != VK_NULL_HANDLE) {
                vkCmdBindDescriptorSets(m_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1, &meshComp.texture->descriptorSet, 0, nullptr);
            }
    ...
    bool VulkanBackend::CreateDescriptorPool() {
        ...
        poolSize.descriptorCount = 100; // max 100 textures for now
...
bool VulkanBackend::CreateTexture(const std::string& filepath, Texture& outTexture) {
        unsigned char* pixels;
        int width, height, channels;
        if (!AssetLoader::LoadTextureData(filepath, pixels, width, height, channels)) {
            return false;
        }
        ...
        AssetLoader::FreeTextureData(pixels);
        ...
        vmaCreateImage(allocator, &imageInfo, &imgAllocInfo, &outTexture.image, (VmaAllocation*)&outTexture.allocation, nullptr);
        ...
        vkCreateImageView(m_device, &viewInfo, nullptr, &outTexture.imageView);
        ...
        vkCreateSampler(m_device, &samplerInfo, nullptr, &outTexture.sampler);

        // Create Descriptor Set
        VkDescriptorSetAllocateInfo dsetAllocInfo{};
        dsetAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        dsetAllocInfo.descriptorPool = m_descriptorPool;
        dsetAllocInfo.descriptorSetCount = 1;
        dsetAllocInfo.pSetLayouts = &m_descriptorSetLayout;

        if (vkAllocateDescriptorSets(m_device, &dsetAllocInfo, &outTexture.descriptorSet) != VK_SUCCESS) {
            std::cerr << "Failed to allocate descriptor set for texture!\n";
            return false;
        }

        VkDescriptorImageInfo descImageInfo{};
        descImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        descImageInfo.imageView = outTexture.imageView;
        descImageInfo.sampler = outTexture.sampler;
    ...
    void VulkanBackend::DestroyTexture(Texture& texture) {
        ...
        if (texture.image != VK_NULL_HANDLE) {
            vmaDestroyImage((VmaAllocator)m_allocator, texture.image, (VmaAllocation)texture.allocation);
